...
library(tidyr)
library(tibble)
library(tidyverse) # BMW: library(tidyverse) already loads dplyr, ggplot2, tidyr, tibble; you don't need both
library(scales)
```
...
```{r}
# summary(green_data) # BMW: You just did this above. Why the repeat?
...
<!-- BMW: Don't use headings for running text. -->
...
reversed <- dictionary %>%
  ...
  pull(Item)
reversed # BMW: Don't print out whole datasets like this in your final scripts.
...
green_rescaled %>%
  ...
  geom_point() + # BMW: Don't use geom_jitter() with continuous variables, use geom_point()
  geom_smooth(method = lm) # BMW: You were missing a + at teh end of the previous line
# This plot shows that Openness positively correlates with Green reputation in both students and non students.

# BMW: These sorts of narrative comments should be in regular markdown text, not code comments.
...
```{r}
# green_rescaled %>%
#   ggplot(aes(x = Green_pomp, color = student), alpha = 1) +
#   geom_bar() +
#   geom_density() +
#   geom_point(aes(y = "trait", color = student), alpha = 1)

# BMW: No, this isn't what was demonstrated in class. See below.

green_rescaled %>% 
  ggplot() + 
  aes(x = Green_pomp, y = student, color = student, fill = student) +
  geom_jitter(width = 0, height = .2, alpha = .4) +
  geom_boxplot(color = "black", fill = "transparent") +
  ggdist::stat_slab(alpha = .5, position = position_nudge(y = .2))
```
...
```{r}
green_rescaled %>%
  select(id, student, Green_pomp, Comp_pomp, Intel_pomp, Open_pomp) %>% # BMW: The select() is unnecessary here.
  group_by(student) %>%
  summarise(across(c(
...
<!-- BMW: You didn't really provide any narrative descriptions of your results. -->
